...
abstract class AbstractIndexDefiner implements IndexDefinerInterface
{
    ...
    public function prepare(string $type, array $document): ?array
    {
        ...
        if (!array_key_exists($type, $definition['mappings'])) {
            return $document;
        }

        if (!array_key_exists('properties', $definition['mappings'][$type])) {
            return $document;
        }

        return $this->prepareObject($definition['mappings'][$type]['properties'], $document);
    }
